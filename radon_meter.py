...
#C2:7B:7C:49:CB:06 FR:R20:SN0440
...
def GetRadonValue():
    ...
    print("Character: " + str(RadonEyeWrite))
    ...
    print ("RAW: " + str(RadonValue))
    RadonValue = struct.unpack('<f',RadonValue[2:6])[0]
    print ("EXTRACT: " + str(RadonValue))
    DevBT.disconnect()
    ...
    if args.mqtt:
        ...
        clientMQTT.connect(args.mqtt_srv, int(args.mqtt_port))
        #print("The type is : ", type(RadonValue))

        if args.mqtt_ha:
            ha_var = json.dumps({"radonvalue": "%0.2f" % (RadonValue)})
           
            #clientMQTT.publish("environment/RADONEYE/"+REkey,ha_var,qos=1)
            print("Publish %s" % ha_var)
            clientMQTT.publish("environment/radoneye", ha_var)
        else:
